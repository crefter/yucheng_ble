...
data class StartEndTimestamp(val start: Long, val end: Long) {
    ...
    companion object {
        ...
        private const val DEFAULT_SERVICE_START_DATE_OFFSET: Long = 1
        ...
        fun service(): StartEndTimestamp {
            val startDate =
                Instant.now().atZone(ZoneId.systemDefault()).toLocalDate().atStartOfDay()
            val utcOffset = ZonedDateTime.now().offset.totalSeconds.toLong()
            val start: Long = (startDate.minusDays(DEFAULT_SERVICE_START_DATE_OFFSET).minusSeconds(utcOffset)
                .toEpochSecond(ZoneOffset.UTC) * 1000)
            val end: Long = (startDate.plusDays(1).toLocalDate().atStartOfDay()
                .toEpochSecond(ZoneOffset.UTC) * 1000)
            return StartEndTimestamp(start, end)
